...
const ABI_METHODS = {
  ...
  is_paused: new algosdk.ABIMethod({
    name: "is_paused",
    args: [],
    returns: { type: "bool" },
  }),
...
/** @deprecated Prefer resolveMarket(chain, poolId, marketId); symbol lookup is ambiguous when the same token exists in multiple pools. */
...
/**
 * Uniquely identifies a lending market: lending pool app id + on-chain market id
 * (the uint64 passed to get_market / deposit / borrow — same as contractId in contracts.json).
 */
export function resolveMarket(chain, poolId, marketId) {
  const m = findMarketByContract(chain, poolId, marketId);
  if (!m) {
    throw new Error(
      `Unknown market poolId=${poolId} marketId=${marketId} on ${chain}. ` +
        `Call get_markets for rows with poolId + marketId (marketId = contractId).`
    );
  }
  return m;
}
...
/** Alias: marketId === contractId for this codebase. */
export function findMarketByPoolMarketId(chain, poolId, marketId) {
  return findMarketByContract(chain, poolId, marketId);
}
