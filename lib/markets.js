...
/**
 * @param {string} chain
 * @param {{ poolId?: number, marketId?: number } | null} filter — if set, both poolId and marketId required
 */
export async function getMarkets(chain, filter = null) {
  if (filter != null && (filter.poolId != null) !== (filter.marketId != null)) {
    throw new Error("get_markets filter requires both poolId and marketId");
  }
  if (filter?.poolId != null && filter?.marketId != null) {
  ...
  for (const md of apiData) {
    ...
    if (filter?.poolId != null) {
      if (md.appId === filter.poolId && md.marketId === filter.marketId) {
        results.push(market);
      }
    } else {
      results.push(market);
  ...
  if (filter?.poolId != null && results.length === 0) {
    throw new Error(
      `Market poolId=${filter.poolId} marketId=${filter.marketId} not found in API data for ${chain}`
    );
  }
...
export async function getMarketOnChain(chain, poolId, marketId) {
...
export async function getIsPaused(chain) {
  ...
  for (const poolId of poolIds) {
    const mr = await simulateABICall(
      chain,
      poolId,
      ABI_METHODS.is_paused,
      []
    );
    pools.push({
      poolId,
      paused: mr.returnValue,
    });
  }

  return { chain, pools, source: "on-chain" };
}
